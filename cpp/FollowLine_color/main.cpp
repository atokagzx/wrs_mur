            ...
            if(abs(obj.x - 160) <= 2 && abs(obj.y - 120) <= 20)
            {
                if(vertex() == 5)
                {
                    ...
                    while(t.elapsed() <= 5000)
                    {
                        ...
                        if(color == -1)
                        {
                            obj = detectColor(mur.getCameraTwoFrame(), 59, 63, 232, 255, 136, 185); //green
                            if(obj.x != -1)
                                color = 0;
                            else
                            {
                                obj = detectColor(mur.getCameraTwoFrame(), 28, 34, 214, 255, 136, 199); //yellow
                                if(obj.x != -1)
                                    color = 1;
                            }
                        }
                        ...
                        {
                            obj = detectColor(mur.getCameraTwoFrame(), 59, 63, 232, 255, 136, 185);
                            set_to_obj(obj, 0, 100, 0);
                        }
                        else if(color == -1)
                        {
                            obj = detectColor(mur.getCameraTwoFrame(), 28, 34, 214, 255, 136, 199);
                            set_to_obj(obj, 0, 100, 0);
                        }
                        mur.setPortD(-15);
                    }
